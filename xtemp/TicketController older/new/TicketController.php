...
class TicketController extends Controller
{
    ...
    public function store(Request $request)
    {
		...
		$userName=User::select('name')->where('id', $recommenderID)->first();               
        ...
        if(!empty($approver_id)){
             ...
             foreach($approver_id as $key=>$file){
                ...
                $approverInfo[]=['ticket_id'=>$TicketID,'user_id'=>$approverID,'user_type'=>2];
                   $ApproverName=User::select('name')->where('id', $approverID)->first();
        ...
        if(!empty($userInfoMail->email)){
        	...
        	try{
        		Mail::send(['html' => 'emails.mail'], $maildata, function ($message) use ($userInfoMail) {
        			...
        			$message->from('partexbs@gmail.com', 'Partex Star Group');
        		});
        	}
        	catch(Exception $e){
        		#continue
    ...
    public function RequestStatusUpdate(Request $request){
        ...
        if(!empty($inputData['formAction'])){
            ...
            if($formAction=='4'){
				...
				if( !empty($udateTicketLog->thistory) ){
					...
					foreach($log as $key => $HistoryInfo){
						...
						if($HistoryInfo['user_id']==Auth::id() && $HistoryInfo['user_action']==0){
							...
							$logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>'Approved','user_action'=>1,'date'=>date('d-m-Y H:i:s')];
						}else{

							$logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>$HistoryInfo['user_status'],'user_action'=>$HistoryInfo['user_action'],'date'=>$HistoryInfo['date']];
						}
            ...
            try{
            	...
            	if(empty($userInfoMail->name)){
            	    ...
            	    $userInfoMailName=$userInfoMail->name;
            	        ...
            	        $message->to("$userInfoMail->email","$userInfoMailName")->subject('You request has been rejected ');
            	        $message->from('partexbs@gmail.com', 'Partex Star Group');
            	});
            }
            catch(Exception $e){
...
$logInfo=array();
  foreach($log as $key => $HistoryInfo){
    if($HistoryInfo['user_id']==Auth::id() && $HistoryInfo['user_action']==0){
    $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>'Rejected','user_action'=>1,'date'=>date('d-m-Y H:i:s')];
}else{
 $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>$HistoryInfo['user_status'],'user_action'=>$HistoryInfo['user_action'],'date'=>$HistoryInfo['date']];
}
...
$logInfo=array();
  foreach($log as $key => $HistoryInfo){
    if($HistoryInfo['user_id']==Auth::id() && $HistoryInfo['user_action']==0){
    $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>'Forward','user_action'=>1,'date'=>date('d-m-Y H:i:s')];
    $userInfo=User::where('id',$inputData['forwardUser'])->first();
     $logInfo[]=['user_id'=>$userInfo->id,'user_name'=>$userInfo->name,'user_type'=>'Forward','user_status'=>'Forward User','user_action'=>0,'date'=>''];
}else{
 $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>$HistoryInfo['user_status'],'user_action'=>$HistoryInfo['user_action'],'date'=>$HistoryInfo['date']];
}
...
$logInfo=array();
  foreach($log as $key => $HistoryInfo){
    if($HistoryInfo['user_id']==Auth::id() && $HistoryInfo['user_action']==0){
    $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>'Appoved And Forward','user_action'=>1,'date'=>date('d-m-Y H:i:s')];
...
$logInfo=array();
  foreach($log as $key => $HistoryInfo){
    if($HistoryInfo['user_id']==Auth::id() && $HistoryInfo['user_action']==0){
    $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>'Appoved And Acknowledgement','user_action'=>1,'date'=>date('d-m-Y H:i:s')];
    $userInfo=User::where('id',$inputData['AcknowledgementBy'])->first();
     $logInfo[]=['user_id'=>$userInfo->id,'user_name'=>$userInfo->name,'user_type'=>'Forward','user_status'=>'Acknowledgement User','user_action'=>0,'date'=>date('d-m-Y H:i:s')];
}else{
 $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>$HistoryInfo['user_status'],'user_action'=>$HistoryInfo['user_action'],'date'=>$HistoryInfo['date']];
}
...
$logInfo=array();
  foreach($log as $key => $HistoryInfo){
    ...
    $userInfo=User::where('id',$userId)->first();
     $logInfo[]=['user_id'=>$userInfo->id,'user_name'=>$userInfo->name,'user_type'=>'Forward','user_status'=>'Assign User','user_action'=>0,'date'=>date('d-m-Y H:i:s')];
}else{
 $logInfo[]=['user_id'=>$HistoryInfo['user_id'],'user_name'=>$HistoryInfo['user_name'],'user_type'=>$HistoryInfo['user_type'],'user_status'=>$HistoryInfo['user_status'],'user_action'=>$HistoryInfo['user_action']];
}
